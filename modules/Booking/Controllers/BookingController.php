    ...
    public function addToCart(Request $request)
{
    ...
    $validator = Validator::make($request->all(), [
        ...
        'coupon_code'  => 'nullable|string', // Add coupon code validation
    ]);
    if ($validator->fails()) {
        return $this->sendError('', ['errors' => $validator->errors()]);
    }
    ...
    // Handle direct booking first if requested
    $directBooking = $request->input('direct_booking', false);

    if ($directBooking && Auth::check()) {
        // First add to cart to generate the booking draft
        $cartResponse = $service->addToCart($request);

        if (!isset($cartResponse->getData()->booking_code)) {
            return $cartResponse; // Return error if cart creation failed
        }

        $bookingCode = $cartResponse->getData()->booking_code;

        // Get the draft booking
        $booking = $this->booking::where('code', $bookingCode)->first();

        if (!$booking) {
            return $this->sendError(__('Booking not found'));
        }
        
        $bookingTotal = $booking->total;  // Total before discount
        if ($coupon) {
            $applyResult = $coupon->applyCoupon($booking, 'add', [
                'total' => $bookingTotal,  
            ...
            if (!$applyResult['status']) {
                return $this->sendError($applyResult['message']);
            }
            $coupon_amount = $applyResult['discount_amount'];
            $booking->coupon_amount = $coupon_amount;
            $booking->total = $bookingTotal - $coupon_amount;
            $booking->total_before_discount = $bookingTotal;
        }

        // Update booking with user information directly
        $user = Auth::user();
        $booking->customer_id = $user->id;
        $booking->email = $user->email;
        $booking->first_name = $user->first_name ?: $request->input('first_name', '');
        $booking->last_name = $user->last_name ?: $request->input('last_name', '');
        $booking->phone = $user->phone ?: $request->input('phone', '');
        $booking->address = $user->address ?: $request->input('address_line_1', '');
        $booking->address2 = $user->address2 ?: $request->input('address_line_2', '');
        $booking->city = $user->city ?: $request->input('city', '');
        $booking->state = $user->state ?: $request->input('state', '');
        $booking->zip_code = $user->zip_code ?: $request->input('zip_code', '');
        $booking->country = $user->country ?: $request->input('country', '');

        // Set payment information
        $booking->gateway = $request->input('payment_gateway', 'offline_payment');

        // Update status directly to processing for direct bookings with offline payment
        if ($booking->gateway == 'offline_payment') {
            $booking->status = 'processing'; // Use string directly to avoid any constant issues
        } else {
            // For other payment methods, handle according to the payment status
            if ($booking->paid >= $booking->total) {
                $booking->status = 'paid';
            } else if ($booking->paid > 0) {
                $booking->status = 'partial_payment';
            } else {
                $booking->status = 'processing';
            }
        }

        // Add other booking meta as needed
        $booking->addMeta('direct_booking', true);
        $booking->addMeta('locale', app()->getLocale());
        $booking->addMeta('how_to_pay', $request->input('how_to_pay', 'full'));
        $booking->save();
        ...
        return $this->sendSuccess([
            ...
            'booking' => [
                ...
                'total' => $booking->total, // Total after discount
                'total_before_discount' => $bookingTotal, 
                'coupon_amount' => $coupon_amount, 
                'start_date' => $booking->start_date,
                'end_date' => $booking->end_date,
            ]
        ], __("Your booking has been confirmed successfully"));
    }

    // If not direct booking, proceed with normal addToCart flow
...
//            $booking->sendStatusUpdatedEmails();
    ...
    public function cancelBooking(Request $request, $id)
    {
        ...
        if (!$this->hasPermission('booking_cancel', $booking)) {
            return $this->sendError(__('You don\'t have permission to cancel this booking'));
        }
        ...
        return $this->sendSuccess(__('Cancel booking successful'));
    }
}
