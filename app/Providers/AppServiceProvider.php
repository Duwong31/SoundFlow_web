...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(Request $request)
    {
        ...
        $this->app->extend('Illuminate\Notifications\Channels\DatabaseChannel', function ($service, $app) {
            return new class extends \Illuminate\Notifications\Channels\DatabaseChannel {
                /**
                 * send noti update for_admin
                 */
                public function send($notifiable, \Illuminate\Notifications\Notification $notification)
                {
                    $dbNotif = parent::send($notifiable, $notification);
                    
                    if ($notification instanceof \App\Notifications\AdminChannelServices) {
                        $dbNotif->for_admin = 1;
                        $dbNotif->save();
                    }
